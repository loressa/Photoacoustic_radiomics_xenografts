...
%We want to see the effect of the bins so find indices of each bin for analysis
binLevels = unique(Tnew.GLbins);
...
for jjj = 1:numel(binLevels)
    toAnalyze = Tnew.GLbins == binLevels(jjj);
    for kk = 1:numel(radInd)
        [intFactorSSE,intFactorNames] = ...
            manualAnova2(Tnew{toAnalyze,radInd(kk)},...
            {Tnew{toAnalyze,3},Tnew{toAnalyze,4},Tnew{toAnalyze,5}},...
            factorNames);
...
for ff = 1:5
    ...
    %include undesired smoothing
    for jjj = 1:numel(binLevels)
        toAnalyze = Tfold.GLbins == binLevels(jjj);
        for kk = 1:numel(radInd)
            [intFactorSSE,intFactorNames] = ...
                manualAnova2(Tfold{toAnalyze,radInd(kk)},...
                {Tfold{toAnalyze,3},Tfold{toAnalyze,4},Tfold{toAnalyze,5}},...
                factorNames);
            radSS = intFactorSSE(1:3)/sum(intFactorSSE);%extracts the feature SS values from anova table and converts them to fractions
            radSS(4) = sum(intFactorSSE(4:end))/sum(intFactorSSE);
            if kk == 1
        ...
        writetable(SSTableFold{jjj,ff},['BinFold',num2str(ff),'.xlsx'],'Sheet',...
            ['BinValueEq',num2str(binLevels(jjj))]);
    end    
end
...
for jjj = 1:numel(binLevels)
    ...
    ba(3).CData = [170 51 119]/255;
    ba(4).CData = [0 0 0]/255;
    legend({'Model','Wavelength','Reconstruction','Error'},'Location','NorthEastOutside')
    ylim([0 1.05]);
